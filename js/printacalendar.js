...
; (function(window, document, $, ZoomPort, undefined) {
  ...
  function initCustomStyleRules() {
    ...
    for (var r in CSS_CLASS_NAMES) {
      if (CSS_CLASS_NAMES.hasOwnProperty(r)) {
          ...
          $('<div/>')
            ...
            .append($('<div style="width: 100%;" \>')
              .append($('<img/>')
                ...
                .attr('src', "/img/cover." + ((monthIndex + 1) % 12 + 1).toString() + ".jpeg")
                .attr('width', "100%")
              )
            )
            .append($('<div/>')
              ...
              .append($('<span/>')
                ...
                .addClass(CSS_CLASS_NAMES.custom)
                .text(((monthIndex != 11) ? 'דניאלה בת שלוש עשרה ' : '') + h[monthIndex])
              )
            )
            .append($('<div/>')
  ...
  function setStyles(styleOpts) {
    for (var r in styleOpts) {
      if (styleOpts.hasOwnProperty(r)) {
        for (var p in styleOpts[r]) {
          if (styleOpts[r].hasOwnProperty(p)) {
